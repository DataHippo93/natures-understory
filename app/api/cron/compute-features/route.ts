...
async function handler(req: NextRequest) {
  ...
  try {
    ...
    const { data: rangeRows } = await admin
      ...
      .limit(1);
    const earliest = rangeRows?.[0]?.sale_date as string | undefined;
    const { data: latestRows } = await admin
      .from('sales_line_items')
      .select('sale_date')
      .order('sale_date', { ascending: false })
      .limit(1);
    const latest = latestRows?.[0]?.sale_date as string | undefined;
    ...
    const { count: invCount } = await admin
      .from('thrive_inventory_snapshot')
...
async function computeSeasonalIndex(
  ...
  // Pull sales rows. Cap to produce category to keep the workload bounded;
  // expand if/when other categories want this signal.
  const { data, error } = await admin
    .from('sales_line_items')
    .select('item_id,quantity,sale_date')
    .eq('category_name', 'Produce [EBT]');
  if (error) throw new Error(`sales_line_items pull: ${error.message}`);
  ...
  for (const row of data ?? []) {
    const item = (row.item_id as string | null) ?? '';
    if (!item) continue;
    const sale = row.sale_date as string | null;
    if (!sale) continue;
    const { isoYear, isoWeek } = isoYearWeek(sale);
    const k = `${isoYear}-${isoWeek}`;
    let weeks = perItemWeek.get(item);
    if (!weeks) { weeks = new Map(); perItemWeek.set(item, weeks); }
    const c = weeks.get(k) ?? { units: 0 };
    c.units += Number(row.quantity ?? 0);
    weeks.set(k, c);
  }
...
function isoYearWeek(dateStr: string): { isoYear: number; isoWeek: number } {
  // ISO week: Thursday-based per ISO-8601
  const d = new Date(dateStr + 'T00:00:00Z');
  const day = d.getUTCDay() || 7;
  d.setUTCDate(d.getUTCDate() + 4 - day); // shift to Thursday of that week
