...
function SectionError({ what, message }: { what: string; message: string }) {
  return (
    <div
      ...
      style={{ background: 'rgba(176,96,96,0.10)', border: '1px solid rgba(176,96,96,0.35)' }}
    >
      <p className="text-sm font-bold" style={{ color: '#d96b6b', fontFamily: 'var(--font-josefin)' }}>
        {what} couldn&apos;t load
      </p>
      <p className="mt-0.5 text-xs font-mono break-words" style={{ color: 'var(--text-muted)' }}>
        {message}
      </p>
    </div>
  );
...
export default async function DashboardPage() {
  // Each data source fails independently — one broken upstream (Clover,
  // Homebase, Thrive warehouse, Home Assistant) degrades its own section
  // instead of taking down the whole dashboard.
  const [kpiRes, shiftRes, coolerRes] = await Promise.allSettled([
    getKPIData(),
    getShiftAnalysisData(7),
    getCoolerDashboard(),
  ]);

  const kpiData = kpiRes.status === 'fulfilled' ? kpiRes.value : null;
  const shiftData = shiftRes.status === 'fulfilled' ? shiftRes.value : null;
  const coolers = coolerRes.status === 'fulfilled' ? coolerRes.value : null;
  ...
  return (
    <Page>
      ...
      {kpiData ? (
        ...
        <SectionError what="Sales &amp; labor KPIs" message={errMsg(kpiRes)} />
      )}
          ...
          <Card>
            ...
            <CardContent>
              {shiftData ? (
                ...
                <SectionError what="Traffic pattern" message={errMsg(shiftRes)} />
              )}
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="p-4">
              ...
              {bestHoursToday.length > 0 ? (
                ...
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                  {shiftData ? 'No quiet hours yet today' : 'Data unavailable'}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-4">
              ...
              {peakHoursToday.length > 0 ? (
                ...
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                  {shiftData ? 'No peak hours yet today' : 'Data unavailable'}
                </p>
              )}
            </CardContent>
          </Card>
